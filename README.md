...
| <a name="output_eks_cluster_endpoint"></a> [eks\_cluster\_endpoint](#output\_eks\_cluster\_endpoint) | Endpoint DNS name for the TFE EKS cluster, derived from the cluster name and current AWS region (`<eks-cluster-name>.<region>.eks.amazonaws.com`). Useful for building a custom EKS node group `user_data` template. |
